...
/**
 * 退款事实收件箱 Mapper（E2E-04 包B，R0-8）。claim/租约/扫描与 {@code RechargeCreditMapper} 的支付事实版本逐条同构，刻意不分叉。
 * SQL 内不含 SQL 注释（Druid WallFilter comment-not-allow）。
 *
 * @author dakang
 * @since 2026-07-29
 */
...
public interface WsRefundEventMapper extends BaseMapper<WsRefundEvent> {
    ...
    /** 按事实幂等键查既有事实。刻意不选 RAW_BODY：原始报文不得顺着 VO 漏到接口。 */
    ...
    /** 同键重复到达的正文摘要比对。返回 0=同一事实号承载两份内容，调用方必须拒绝并转人工。 */
    ...
    /**
     * 认领事实：1待处理 / 4到期待重试 / 2租约已过期 → 2处理中。三类候选与 {@link #selectDueEventIds} 同构；0 行=已被认领或已处理，必须放弃。
     * 「2处理中且租约过期」一支是崩溃恢复的全部：没有它，claim 后挂掉的退款永远无人推进。
     */
    ...
    /** 扫描待推进的退款事实（Worker 专用，只出 ID）。宽松预筛，并发安全由 {@link #claim} 的 CAS 保证；三类候选与 claim 同构。 */
    ...
    /**
     * 记录「同键不同正文」异常，只写错因不动处理状态：若 park 既有事实，任何知道事实键的人发垃圾正文即可把合法退款冻进人工队列（拒绝服务路径）。
     * 该拒绝的是本次到达（调用方抛出）；异常写进 LAST_ERROR 留痕供检索。
     */
