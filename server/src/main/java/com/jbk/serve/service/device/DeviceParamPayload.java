...
/**
 * 设备参数报文的唯一解析与校验实现（REQ-213-S1）：单发与批量必须共用本类，任一处放松即
 * 畸形报文入口。两层校验：结构校验恒生效（扁平 JSON、键名受限、只允许标量、禁嵌套禁 null、
 * 键数与串长上限）；登记校验登记即生效（键在 ws_device_param_def 启用时校验类型/值域/枚举），
 * 未登记键放行但标记——单调收紧：空注册表即今天已验收行为，登记只会更严不会更松
 * （业务参数键在厂家答复 V-12.3 前保持零登记）。与 B20 未登记故障码 fail-closed 取向不同
 * 且刻意：故障码是设备上报（未知即看不懂设备状态），参数是平台受控下发且设备自己会拒绝
 * 不认识的键。
 */
