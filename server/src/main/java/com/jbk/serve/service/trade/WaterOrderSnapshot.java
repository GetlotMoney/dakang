...
/**
 * 取水订单冻结快照（{@code ws_order.PACKAGE_SNAP}）的唯一构造与解析实现——同一份快照在
 * 下单扣款、指令准备、出水结算三个时点被读，散写解析会漂移出宽严不一的口径。
 *
 * <p>两种读法不可互换：{@link #parseStrict} 供下单/指令准备，任何缺失或非法 fail-closed；
 * {@link #readUnitPriceLenient} 供结算，历史单缺单价字段按回退口径（单价 0）留痕，
 * 但字段存在却非法属篡改信号必须抛出，不得用全额退款掩盖。</p>
 *
 * @author dakang
 * @since 2026-08-03
 */
