 ...
 *
 * <p>入参是原始值而非读模型 Vo：写路径（异常核账、售后执行）手里只有 PO，吃 Vo 会逼出第二份
 * 矩阵并漂移（铁律⑤）。fail-closed：任一证据缺失/不一致都返回原因，调用方只呈现「数据异常」，
 * 返回 null 才代表全部核验通过。状态数字一律取自枚举，本类不定义状态字面量。</p>
 *
 * @author dakang
 * @since 2026-07-29
 */
public final class OrderCommandVerifier {
    ...
    /**
     * 只接受当前真实状态机能产生的订单/指令状态与时间证据组合。
     *
     * <p>4已完成/7已退款有两种合法来源：① 正常结算——指令必为 4执行成功；② 取水异常核账确认
     * （E2E-04 包A）把 6 推进到 4/7 而刻意不改写指令（指令是核账证据），其指令停在 5失败/6超时。
     * 放宽仅限 afterSaleConfirmed 且订单落 4/7 这一格（额外接受 5/6 终态指令，须有指令完成时间），
     * 其余证据要求一律不动。</p>
     *
     * @param orderStatus        订单状态(1341)
     * @param orderFinishTime    订单完成时间
     * @param afterSaleConfirmed 是否已经过取水异常核账确认（调用方批量带出，本类不查库防 N+1）
