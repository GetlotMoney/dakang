...
/**
 * 小程序售后只读契约（E2E-04 包E）。售后动作全部是 PC 运营端行为，小程序不提供任何触发入口；
 * 金额与水量由服务端算好冻结，端上只格式化、不加减不合计（合计取服务端 refundAmount 原值）；
 * 完成口径只认服务端 actionStatus=3（补送单生成 ≠ 补送完成，包C 结论）；取消资格由服务端判定下发。
 * 字典 1370/1371/1372/1373 与后端 AfterSaleEnum、ws_refund 同源，未登记值一律 fail-closed 隐藏。
 */
...
/**
 * 面向用户的售后进度（只读）。字段名与 PC 台账行 `AdminAfterSaleActionItemVo` 一致，
 * `resendOrderNo/resendTaskNo` 是补送结果（`RESULT_ORDER_ID/RESULT_TASK_ID`）在用户侧的业务号投影，
 * `refundSource` 取自 `ws_refund`。用户侧不下发运营人员、重试次数、原始退款报文等内部字段。
 */
...
/** 待接单取消资格。唯一来源是服务端判定（包A 事务），页面不自算；缺省（未下发）即不显示入口。 */
...
/** 退款来源文案（1373）。R0-8：模拟退款不得渲染成「微信退款」，两个取值各自照实展示，未登记值不解释。 */
...
/**
 * 售后结局文案。未到 `actionStatus=3` 一律"处理中/待补送"，不得出现"成功/已到账/已补送"；
 * 补送(4) 的完成条件是配送员回签（包C `completeOnSigned`），与资金返还分开措辞。
 */
...
/** 返还额度展示行：逐项格式化服务端原值，不求和不折算；合计缺失就不展示，不用三项相加顶上。 */
...
/** 「补送」标识判定：只认两个服务端值相等（本单订单号 === 售后回填的补送单号），不按金额/回收桶特征猜。 */
...
/**
 * 售后进度归一化。任一必需字段缺失/额度畸形/枚举越界 → 整块返回 undefined 隐藏区块；
 * 刻意不抛错（附加证据不该让整页 404），也不允许"部分字段能用就先渲染"。
 */
export function normalizeAfterSaleProgress(value: unknown): AfterSaleProgress | undefined {
  ...
  if (!afterSaleNo
    // 上界随 1370 字典放到 4：留在 3 会让充值退款的进度被整条静默丢掉
...
/**
 * 取消资格归一化：接受布尔与结构化 `{allowed, reason}` 两种下发形态，都缺即 undefined 不显示入口。
 * 只有显式 true 才算允许，1/'true' 等真值不算。
 */
...
export interface AfterSaleApi {
  /** 待接单取消配送订单。返回服务端文案并原样展示：资金段在途时不得显示成"已退款"。 */
...
const realAfterSaleApi: AfterSaleApi = {
  async cancelPendingDeliveryOrder(orderNo) {
    ...
    return withRealSession(async () => {
      // 归属/待接单/资金返还全部在服务端事务内裁决，这里只发起并转述结论
...
/** 售后动作挂在 delivery 域下，不新增运行模式域：本包没有独立于配送域的真实端点。 */
