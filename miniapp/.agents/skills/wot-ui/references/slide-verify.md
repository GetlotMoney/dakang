...
通过 `width` 和 `height` 属性自定义滑动条的宽度和高度。通过 `icon-size` 和 `success-icon-size` 属性设置对应图标大小。
...
```html
<wd-slide-verify ref="slideVerifyRef" /> <wd-button @click="handleReset">重置</wd-button>
```
