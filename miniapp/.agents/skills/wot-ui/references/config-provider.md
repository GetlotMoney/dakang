...
:::tip值得注意的是，开启 `Wot` 的深色模式只会影响 `Wot` 组件的 `UI`，并不会影响全局的文字颜色或背景颜色，你可以参考以下 `CSS` 来设置一些全局样式：:::
...
:::tip 注意：ConfigProvider 仅影响它的子组件的样式，不影响全局 root 节点。:::
...
:::tip 若存在改变 pages.json 的插件，需要将 UniKuRoot 放置其后 :::
...
:::tip 在 App.ku.vue 中标签 `<KuRootView />` 代表指定视图存放位置 :::
