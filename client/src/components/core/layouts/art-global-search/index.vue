...
<script lang="ts" setup>
  ...
  const targetKey = (item: AppRouteRecord): string => item.path || String(item.meta.link || '')

  const hasAuthorizedTarget = (items: AppRouteRecord[], target: AppRouteRecord): boolean => {
    ...
    return items.some(
      (item) =>
        targetKey(item) === key ||
        (item.children?.length ? hasAuthorizedTarget(item.children, target) : false)
    )
  }

  const sanitizeSearchHistory = (): void => {
    // 动态路由尚未初始化时不动持久化数据，避免登录过程误清空。
    if (!routeList.value.length) return
    const authorized = historyResult.value.filter((item) =>
      hasAuthorizedTarget(routeList.value, item)
    )
    if (authorized.length !== historyResult.value.length) {
      userStore.setSearchHistory(authorized)
    }
  }

  watch(routeList, sanitizeSearchHistory, { immediate: true })
  ...
  const search = (val: string) => {
    if (val) {
      searchResult.value = flattenAndFilterMenuItems(routeList.value, val)
    } else {
      searchResult.value = []
    }
  }
  ...
  const flattenAndFilterMenuItems = (items: AppRouteRecord[], val: string): AppRouteRecord[] => {
    ...
    const flattenAndMatch = (item: AppRouteRecord, ancestorTitles: string[] = []) => {
      if (item.meta?.isHide) return

      const itemTitle = formatMenuTitle(item.meta.title)
      const titlePath = [...ancestorTitles, itemTitle]
      const searchableTitle = titlePath.join(' / ').toLowerCase()

      if (item.children && item.children.length > 0) {
        item.children.forEach((child) => flattenAndMatch(child, titlePath))
        return
      }

      if (
        searchableTitle.includes(lowerVal) &&
    ...
    items.forEach((item) => flattenAndMatch(item))
    return result
  }
  ...
  const searchGoPage = (item: AppRouteRecord) => {
    // 搜索历史会持久化；跳转前再按当前账号的完整授权路由树复验。
    if (!hasAuthorizedTarget(routeList.value, item)) {
  ...
  const addHistory = (item: AppRouteRecord) => {
    ...
    const hasItemIndex = historyResult.value.findIndex(
      (historyItem: AppRouteRecord) => targetKey(historyItem) === itemKey
    )
