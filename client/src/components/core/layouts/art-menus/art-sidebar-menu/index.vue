        ...
        <ul>
          <li v-for="menu in firstLevelMenus" :key="menu.path" @click="handleMenuJump(menu, true)">
            ...
            >
              <div
                :class="{
                  'is-active': isDisplayMenuActive(menu, activeMenuPath)
                }"
                :style="{
...
<script setup lang="ts">
  ...
  } from '@/router/core/MenuDisplayProjector'
  ...
  // 路由相关
  // 菜单数据
  const allMenus = computed(() => useMenuStore().menuList)
  const activeMenuPath = computed(() =>
    resolveDisplayMenuActivePath(String(route.meta.activePath || route.path), allMenus.value)
  )
  const routerPath = computed(() => activeMenuPath.value)
  ...
  const menuList = computed(() => {
    ...
    if (isIframe(route.path)) {
      return findIframeMenuList(route.path, allMenus.value)
    }

    // 展示菜单可能与技术路由树层级不同，递归定位当前所属的顶级展示项。
    const currentMenu = allMenus.value.find((menu) =>
      isDisplayMenuActive(menu, activeMenuPath.value)
    )
