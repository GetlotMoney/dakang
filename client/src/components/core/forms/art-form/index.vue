...
<script setup lang="ts">
  ...
  export interface FormItem {
    ...
    /** 是否隐藏该表单项 */
    hidden?: boolean
    /** 是否显示该表单项（show=false 等同于 hidden=true，区别在于条件表达式语义） */
    show?: boolean
    /** 表单项占据的列宽，基于24格栅格系统 */
    span?: number
  ...
  const getSanitizedOutput = () => {
    // 显式标注 Record 类型：sanitizeOutputValue 返回 {} 时按 string 索引会报 TS7053（基座自带问题）
    const result: Record<string, any> = sanitizeOutputValue(cloneModelValue(modelValue.value)) || {}
    for (const item of props.items) {
      if (item.transform && result[item.key] !== undefined) {
        result[item.key] = item.transform(result[item.key])
      }
    }
    return result
  }
  ...
  const visibleFormItems = computed(() => {
    return props.items.filter((item) => !item.hidden && item.show !== false)
  })
  ...
  // 解构 props 以便在模板中直接使用
  const { span, gutter, labelPosition, labelWidth, validateOnRuleChange } = toRefs(props)
</script>
