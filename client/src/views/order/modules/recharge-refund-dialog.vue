<!--
  充值/购卡退款（REQ-061，售后来源 4充值退款）：只展示 /refund/recharge/preview 下发的依据并
  提交「订单ID + 受理说明」，不参与任何金额计算：可退金额按冻结公式从权益批次折算，冲减量取批次剩余，
  卡是否注销由 CardClosureRule 判定，全部由 preview 下发。
  受理后钱原路退回用户支付账户，不可撤销，故必须先预览——运营点下去之前必须看到三件事：
  退多少钱、卡上少多少权益、这张卡会不会被注销。
  两个金额是不同维度、分块展示：退回支付账户=按实付折算（赠送与已消费不退），
  从卡上收回=批次剩余（含未消费赠送）。
-->
<template>
  <ElDialog
