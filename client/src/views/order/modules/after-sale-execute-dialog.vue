<!--
  售后高风险操作二次确认（E2E-04 包E）。执行返还 / 发起退款 / 生成补送共用一个弹窗。

  三条硬约束：
  ① 弹窗里没有任何可编辑的金额、水量或目标状态输入。四元额度在售后动作登记时按订单快照
     算定并冻结，执行事务只读它并把四列写进 CAS；此处只把服务端下发的额度原样摊开给运营核对。
  ② 确认动作要求逐字重敲售后号。它不是装饰性的“请输入 DELETE”——afterSaleNo 是接口的共键复核
     字段，服务端会比对 id 与售后号是否指向同一行：一张停在浏览器里的过期台账（他人已处理、
     翻页后行序变化）足以让运营点中另一笔合法的待执行返还，而状态 CAS 与唯一键都拦不住，
     因为那确实是“对某行的合法执行”。重敲即是让运营亲眼确认自己执行的是哪一笔。
  ③ 权限由调用方在打开前判定，本弹窗再判一次；两处都不构成授权依据，后端 @SaCheckPermission 才是。
-->
<template>
  <ElDialog
