...
// 将计数字段归一化为非负数值。后端全局 Jackson 把 Long/long 序列化为字符串（防 JS 精度丢失，见
// server JacksonConfig），因此 total、current、size 等可能以数值字符串（如 "402"）返回。number 直接采用；
// 非空数值字符串按 Number() 转换后为有限非负数才采用；空串/空白/null/NaN/Infinity 一律视为无效返回 undefined。
...
function extractTotal(obj: Record<string, unknown>, records: unknown[], fields: string[]): number {
  for (const field of fields) {
    if (field in obj) {
      const total = coerceCount(obj[field])
      if (total !== undefined) {
        return total
      }
    }
...
function extractPagination(
  ...
  for (const src of sources) {
    for (const field of currentFields) {
      if (field in src) {
        const current = coerceCount(src[field])
  ...
  for (const src of sources) {
    for (const field of sizeFields) {
      if (field in src) {
        const size = coerceCount(src[field])
        if (size !== undefined) {
          result.size = size
          break
        }
