...
当前全部运行在受控模拟环境上。业务链的逐条范围与完成状态见
`docs/requirements/demo-business-chain-matrix.md`——本文只写指针，不复制状态与计数。

## 系统范围

- **PC 运营后台**：运营总览、水站、设备中控、水种套餐、用户、订单中心 + 系统管理。
...
真机联调另需 `miniapp/env/.env.development.local`（`.local` 后缀才被忽略、不入库；模板见同目录
`.env.development.example`）：填本机局域网 IP 与自己的小程序 AppID。
**不要写进 `.env.development`**——那份是已入库的公开安全档，写进去会随提交泄露。
...
以下三个开关默认关闭，**误开会产生真实资金或账号后果**，仅限本机演示与受控验收：

- `MINI_PAYSIM_ENABLED`：模拟支付；它同时开启测试登录端点，而后端绑 `0.0.0.0`。
- `MINI_REFUNDSIM_ENABLED`：模拟退款——误开会把模拟退款按真实微信退款记账。
- `MINI_PHONELESS_REGISTER_ENABLED`：允许无手机号建号，是**单向门**（产生 NULL 手机号账号后库结构无法改回）。
...
| 查充值/异常取水的接口契约 | `docs/contracts/L2-recharge-contract-v2.md`、`docs/contracts/H3-water-exception-contract.md` |
